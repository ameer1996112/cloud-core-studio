...
-- Assign a plan to a member: creates a member_plans row and grants the credits atomically
...
-- Generate a class from a template at a specific start_at
...
BEGIN
  ...
  INSERT INTO public.classes(title, instructor_id, start_at, end_at, capacity, room, energy, credit_cost, cancellation_window_hours, status)
    VALUES (
      v_t.title, v_t.default_instructor_id, p_start_at,
      p_start_at + (v_t.default_duration_minutes || ' minutes')::interval,
      v_t.default_capacity, v_t.default_room, v_t.default_energy,
      v_t.default_credit_cost, v_t.default_cancellation_window_hours, 'scheduled'
