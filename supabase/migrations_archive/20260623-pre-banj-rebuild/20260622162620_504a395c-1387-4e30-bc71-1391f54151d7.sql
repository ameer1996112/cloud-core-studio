...
BEGIN
  ...
  -- Existing receipt check (drives idempotency)
  SELECT * INTO v_existing_receipt FROM public.receipts WHERE payment_id = p_payment_id;
  v_has_existing := FOUND;

  -- Idempotent: already confirmed AND receipt exists → return same receipt
  ...
  -- Guard: refunded / cancelled / failed payments are terminal — do not silently flip back to paid
  ...
  -- Mark payment confirmed (idempotent COALESCEs on confirmed_at / paid_at)
  ...
  -- Activate plan exactly once: only if attached AND no member_plan yet on the payment
  ...
  -- Issue receipt only when none exists (unique(payment_id) backstops races)
