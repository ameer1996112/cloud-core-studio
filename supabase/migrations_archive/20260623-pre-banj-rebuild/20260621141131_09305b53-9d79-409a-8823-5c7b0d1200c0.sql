...
-- Instructors/admins can read members rostered on their classes via care brief; allow read for instructor/admin role
...
CREATE POLICY "Instructors read class bookings" ON public.bookings FOR SELECT TO authenticated
  USING (public.has_role(auth.uid(),'instructor') OR public.has_role(auth.uid(),'admin'));
-- No INSERT/UPDATE policies: writes flow exclusively through book_class() SECURITY DEFINER.
