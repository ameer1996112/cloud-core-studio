...
BEGIN
  ...
  IF now() > v_deadline THEN
    RETURN jsonb_build_object('status','window_passed','deadline', v_deadline);
  END IF;

  UPDATE public.bookings SET status='cancelled' WHERE id = p_booking_id;
  UPDATE public.classes SET booked_count = GREATEST(0, booked_count - 1) WHERE id = v_class.id;
  UPDATE public.attendance_records SET status='cancelled', marked_by=p_actor_id, marked_at=now()
    WHERE booking_id = p_booking_id;
  IF v_booking.credit_cost > 0 THEN
    UPDATE public.members SET remaining_credits = remaining_credits + v_booking.credit_cost
  ...
  -- auto-promote first waitlist entry (best-effort, leaves status='waiting' if booking fails)
  SELECT * INTO v_next FROM public.waitlist_entries
    WHERE class_id = v_class.id AND status='waiting'
    ORDER BY created_at ASC LIMIT 1 FOR UPDATE SKIP LOCKED;
  IF FOUND THEN
    UPDATE public.waitlist_entries SET status='ready', promoted_at=now() WHERE id = v_next.id;
  END IF;
...
BEGIN
  ...
  SELECT * INTO v_existing FROM public.waitlist_entries
    WHERE class_id=p_class_id AND member_id=p_actor_id AND status IN ('waiting','ready') LIMIT 1;
  IF FOUND THEN
  ...
  INSERT INTO public.waitlist_entries(class_id, member_id, status)
    VALUES (p_class_id, p_actor_id, 'waiting')
...
BEGIN
  ...
  UPDATE public.waitlist_entries SET status='left' WHERE id=p_entry_id;
