...
-- ===== Phase 1: Studio Management Console foundation =====

-- 1. Additive column changes (backwards compatible)
...
-- Allow admins to write classes/instructors directly (RPCs are still preferred,
-- but admins editing low-risk fields via PostgREST is fine since RLS gates it).
...
-- 4. attendance_records (one per booking)
...
-- Replace book_class to also write ledger + attendance seed (member behavior unchanged)
...
-- Admin: create booking for any member
...
-- Admin: cancel a booking (refunds credit + frees seat)
...
-- Staff (instructor or admin): mark attendance
...
-- Admin: waitlist promote (creates booking, marks entry promoted)
