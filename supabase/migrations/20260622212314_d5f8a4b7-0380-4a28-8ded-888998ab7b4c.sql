...
CREATE POLICY "Authenticated can read active program types"
  ON public.program_types FOR SELECT TO authenticated
  USING (active = true OR public.has_role(auth.uid(), 'admin'));
CREATE POLICY "Admins can insert program types"
  ON public.program_types FOR INSERT TO authenticated
  WITH CHECK (public.has_role(auth.uid(), 'admin'));
CREATE POLICY "Admins can update program types"
  ON public.program_types FOR UPDATE TO authenticated
  USING (public.has_role(auth.uid(), 'admin'))
  WITH CHECK (public.has_role(auth.uid(), 'admin'));
CREATE POLICY "Admins can delete program types"
  ON public.program_types FOR DELETE TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));
