...
-- 5. Atomic confirm + activate + receipt
...
BEGIN
  ...
  -- Idempotent: if already confirmed and receipt exists, return it.
  ...
  -- Update payment: mark confirmed, attach provider data
  ...
  -- Activate plan if one is attached and no member_plan yet
  ...
  -- Issue receipt if not already present
  IF NOT FOUND OR v_existing_receipt.id IS NULL THEN
