...
-- 1. members: prevent self-modification of credit/attendance columns
REVOKE UPDATE (remaining_credits, attendance_count, last_visit_at) ON public.members FROM authenticated;

-- 2. instructors: add admin-only write policies
...
-- 3. bookings: explicitly deny direct INSERT/UPDATE/DELETE from clients.
-- All booking lifecycle must go through SECURITY DEFINER RPCs (book_class, admin_*).
...
-- 4. SECURITY DEFINER admin functions: rewrite to take explicit actor id,
-- then revoke EXECUTE from public roles so signed-in users cannot call them
-- directly through PostgREST. They will be invoked only by service_role via
-- the server-only admin client, with the actor id supplied by server functions.
...
-- Drop the old single-arg admin signatures so they cannot be called from PostgREST
...
-- Lock down admin functions: only service_role can execute (server fns will call via admin client)
