...
BEGIN
  ...
  FOR v_item IN SELECT value FROM jsonb_array_elements(p_materializations) LOOP
    ...
    IF v_template.channel = 'whatsapp' AND NOT EXISTS (
      SELECT 1 FROM public.whatsapp_template_deployments w WHERE w.template_name = p_template_key
  ...
  INSERT INTO public.concierge_decisions(studio_id,journey_instance_id,intent_id,communication_recipient_id,decision_key,
    policy_version,automation_config_version,template_id,template_version,locale,reason_codes,competing_action_ids,simulated)
  ...
  IF v_decision_id IS NULL THEN
    SELECT id INTO v_decision_id FROM public.concierge_decisions WHERE studio_id = p_studio_id AND decision_key = p_decision_key;
    RETURN QUERY SELECT v_decision_id,'duplicate'::text,NULL::text; RETURN;
  END IF;
  ...
  FOR v_item IN SELECT value FROM jsonb_array_elements(p_materializations) LOOP
    ...
    IF v_message_id IS NULL THEN SELECT id INTO v_message_id FROM public.messages WHERE idempotency_key = 'concierge:' || v_decision_id || ':' || (v_item->'delivery'->>'channel'); END IF;
