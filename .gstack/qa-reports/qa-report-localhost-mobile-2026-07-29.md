...
**Remediation verification:** Production build and lint passed. Focused auth, localization, schedule, and class-card tests passed. At 390×844, the signed-out auth form was ready on the first production snapshot; body text measured 16px; muted text used `#5f6b7e`; and every audited auth target measured at least 44×44px. Hebrew invalid-email feedback rendered inline in Hebrew, and signup exposed `name`, `tel`, `email`, and `new-password` autofill tokens.
