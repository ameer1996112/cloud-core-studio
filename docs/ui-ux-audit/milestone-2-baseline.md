...
Public checkout screenshots can be taken locally without submitting payment. Authenticated package and account states need an approved safe member and mocked/non-production payment/deletion outcome. No credentials or safe fixture were provided at baseline, so those views are explicitly blocked rather than substituted with a production interaction.
