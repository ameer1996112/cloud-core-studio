...
- Kick maintenance only after commit. A kick is an acceleration hint, not the durability mechanism.
...
- Cloud Run IAM Invoker is granted only to the dedicated Tasks and Scheduler service accounts.
...
| WhatsApp | Mac-local claim/report | existing OpenWA bridge; explicit consent required |

Template rendering remains in the existing versioned catalogs. The outbox captures locale and template metadata, while the materialized message keeps the historical rendered content needed for auditability.
