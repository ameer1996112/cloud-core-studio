...
Cloud & Core 1.0.7 build 8 reports its native version through `@capacitor/app`.
...
2. Confirm version 1.0.7 is publicly downloadable from the App Store.
3. Enable the minimum version in Cloud Run:
   ...
   gcloud run services update cloud-core-studio \
     ...
     --region me-west1 \
     --update-env-vars IOS_FORCE_UPDATE_ENABLED=true,IOS_MINIMUM_APP_VERSION=1.0.7
   ```

4. Open an older installed build and confirm it shows the App Store gate.
5. Open version 1.0.7 and confirm the app opens normally.
