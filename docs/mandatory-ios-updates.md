...
Cloud & Core native builds report their version through `@capacitor/app`. The TestFlight
candidate in this repository is 1.0.8 build 10. The web client checks a public Cloud Run policy
at startup and when the app returns to the foreground.

## Safe release order

1. Keep enforcement disabled while the iOS build is in TestFlight or App Review.
2. Confirm version 1.0.8 is publicly downloadable from the App Store.
3. Enable the minimum version in Cloud Run:
   ...
   gcloud run services update cloud-core-studio \
     ...
     --region me-west1 \
     --update-env-vars IOS_FORCE_UPDATE_ENABLED=true,IOS_MINIMUM_APP_VERSION=1.0.8
   ```

4. Open an older installed build and confirm it shows the App Store gate.
5. Open version 1.0.8 and confirm the app opens normally.
