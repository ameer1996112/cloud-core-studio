...
Run: `bun tools/ui-audit/interaction-checks.ts && bun test tests/unit/accessibilityEvidence.test.ts`
...
Run: `bun run build && bun run ui-audit:validate && bun run ui-audit:capture && bun run ui-audit:a11y && bun tools/ui-audit/bundle-budget.ts && bun tools/ui-audit/lighthouse.ts && bun tools/ui-audit/interaction-checks.ts`
