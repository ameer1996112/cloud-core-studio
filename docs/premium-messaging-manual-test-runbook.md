...
- Canonical migration through `20260722120000_premium_messaging_journey_tuning.sql` is applied.
...
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `member_welcome`                 | Create a new active allowlisted member, then separately activate an existing inactive member. Repeat updates and test upcoming booking, credits, and neither. | Within five minutes: in-app, approved-locale WhatsApp, email; exactly one welcome per member. Primary action opens the upcoming class, schedule, or packages. |
...
| `class_reminder_planning`  | Create a booked class whose cancellation deadline is two hours away, then sweep. Repeat once with and once without an active push installation.                                               | In-app + push when push exists; WhatsApp only when push is unavailable. Cancelling the booking/class before dispatch changes pending deliveries to `cancelled`.                            |
| `class_reminder_final`     | Create a booked class two hours away and sweep. For a class before 10:30, test 20:00 on the previous day.                                                                                     | In-app + WhatsApp once; no duplicate push. Cancelled bookings/classes do not receive it.                                                                                                   |
...
| `class_open_spots`         | Create a visible scheduled class 2–24 hours away at no more than 70% capacity; create 14 eligible staff/beta profiles with different attendance affinity; sweep.                              | The ten best matches receive in-app/push. No WhatsApp/email. At 85% capacity pending sends cancel. Daily/weekly caps and the 10% control group apply.                                      |
| `class_recommendation`     | Give the staff member matching history and consent, no future booking/planning message for seven days, two successfully sent but unopened growth pushes, and leave matching classes unbooked. | One message offers the best one or two lessons. In-app/push first. A single WhatsApp/7 days is eligible only after the push-evidence gates; zero credits replaces `Book now` with package. |
...
| Event                      | Domain action to trigger automatically                                                                                  | Expected channels and result                                                                                                          |
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| `payment_request_received` | Create a pending studio payment or a requested package request.                                                         | Immediate in-app and email with payment action; no push interruption.                                                                 |
...
| `payment_confirmed`        | Change a normal pending payment to `paid`, then repeat after first changing a second payment to `failed`.               | Normal: in-app, quiet push, email. Recovered failure: same plus WhatsApp. Any queued pending escalation becomes `cancelled`.          |
...
| `retention_reminder`         | Set `last_visit_at` to 21 days ago, then 30 days ago, with marketing/WhatsApp consent. Sweep at each stage. | Day 21: in-app + caring push. Day 30: in-app + personal WhatsApp. Three ignored growth sends create a 30-day cooldown; no guilt copy.                           |
...
- Confirm a control-group assignment stores `premium_growth_v2/control` and produces no growth
  delivery; never force treatment for a real customer during rollout measurement.
