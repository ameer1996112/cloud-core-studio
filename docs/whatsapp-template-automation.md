...
The template catalog is immutable application code in `src/lib/messageTemplateCatalog.ts`. Generated Meta JSON lives under `whatsapp/templates/v2/{he,ar,en}`. The current v2 catalog contains 20 semantic names and 60 locale variants. The welcome template is `UTILITY`; recommendation, personal-return, and weekly-schedule templates are `MARKETING`. The legacy v1 JSON remains untouched for rollback/history.
