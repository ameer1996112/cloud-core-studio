...
The template catalog is immutable application code in `src/lib/messageTemplateCatalog.ts`. Generated Meta JSON lives under `whatsapp/templates/v2/{he,ar,en}`. The current v2 catalog contains 16 semantic names and 48 locale variants. The legacy v1 JSON remains untouched for rollback/history.
