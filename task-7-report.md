...
| Local production server smoke (`PORT=4173 bun run start`) | PASS after localhost binding was permitted. `/` returned 200; the branded asset returned `200 image/webp` with `public, max-age=3600`; protected `/admin/messages`, `/admin/messages/concierge`, and `/admin/templates` returned 307 to `/auth` with safe `returnTo` values. |
...
| Existing `concierge-e2e` tag smoke | PARTIAL. `/` returned 200 and protected routes returned 307 to `/auth`; the branded asset returned `404 text/html`. The tag points to an older revision, not a newly built exact-commit revision. |
