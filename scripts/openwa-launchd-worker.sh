...
if "$NODE_BIN" "$ROOT_DIR/scripts/openwa-local-worker.mjs" "${ARGS[@]}" >"$TMP_RESPONSE_FILE" 2>&1; then
  ...
  trap - EXIT
  log_success "ok limit=$LIMIT dryRun=${OPENWA_WORKER_DRY_RUN:-0} testPhoneOnly=${OPENWA_WORKER_TEST_PHONE_ONLY:-0} nodeBin=$NODE_BIN"
  exit 0
fi

mv "$TMP_RESPONSE_FILE" "$LAST_RESPONSE_FILE"
trap - EXIT
log_error "worker_failed limit=$LIMIT dryRun=${OPENWA_WORKER_DRY_RUN:-0} testPhoneOnly=${OPENWA_WORKER_TEST_PHONE_ONLY:-0} nodeBin=$NODE_BIN response_file=$LAST_RESPONSE_FILE"
exit 1
