      ...
      <div className="bg-white border border-gold/25 rounded-2xl p-5 shadow-[0_4px_16px_var(--cc-alpha-navy-02)] grid grid-cols-2 md:grid-cols-4 gap-4 text-start">
        <div>
          ...
          <p className="font-display text-sm font-semibold text-navy mt-1">
            {hasSelectedTime ? (
              <>
                <BidiValue kind="localized-date">{formatSessionDate(startDate, lang)}</BidiValue> ·{" "}
                <BidiValue kind="time-range">{formatSessionTime(startDate, lang)}</BidiValue>
              </>
            ) : (
...
function PremiumClassEditor({
  ...
  return (
    ...
    >
      <header className="rounded-3xl border border-gold/25 bg-[linear-gradient(145deg,var(--cc-alpha-white-98),var(--cc-admin-form-surface-end))] p-5 shadow-[0_18px_50px_var(--cc-alpha-navy-07)] sm:p-7">
        ...
        <div className="grid gap-4 sm:grid-cols-[1fr_auto]">
          ...
          <PremiumField label={t("admin.classes.time")}>
            <div
              ...
              dir={bidiDirectionFor("time-range")}
            >
              <select
