...
import { Calendar, MessageCircle, Sparkles, Clock, ArrowRight, Megaphone } from "lucide-react";
...
function MemberHome() {
  ...
  const contactText = t("member.contactMessage", {
    studio: settings?.studio_name ?? "Cloud & Core",
    member: data?.member?.name ?? t("member.friend"),
  });
  const contactUrl = waUrl({
    to: settings?.whatsapp_number ?? settings?.public_phone ?? null,
    text: contactText,
  });
    ...
    >
      ...
      {fatalError ? (
        ...
        nextBooking ? (
          ...
          <MemberSection
            id="recommended"
            eyebrow={t("member.recommendedForYou")}
            title={t("member.keepPracticeMoving")}
            action={
      ...
      {hasHomeData ? (
        <>
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="member-home-contact-link"
          >
            <span className="inline-flex items-center gap-2">
              <MessageCircle className="h-4 w-4 text-gold" aria-hidden />
              {t("member.contactStudio")}
            </span>
            <ArrowRight className="h-3 w-3 text-gold directional-icon-forward" aria-hidden />
          </a>
        </>
      ) : null}
