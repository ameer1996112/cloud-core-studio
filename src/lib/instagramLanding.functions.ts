...
type InstagramKidsPlanRow = {
  code: unknown;
  credits_per_period: unknown;
  name: unknown;
  period_days: unknown;
  price: unknown;
  recurring_available: unknown;
  total_periods: unknown;
};
...
export const getInstagramLandingData = createServerFn({ method: "GET" }).handler(
  async (): Promise<InstagramLandingData> => {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const [settingsResult, adultPlansResult, kidsPlansResult, kidsProgramResult] =
      await Promise.all([
        supabaseAdmin
        ...
        // The checked-in Supabase types predate the kids-package migration; keep the escape hatch local.
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (supabaseAdmin as any)
          .from("kid_aerial_packages")
          .select(
            "code,name,price,credits_per_period,total_periods,period_days,recurring_available",
          )
          .eq("active", true)
    ...
    const kidsPlans = kidsPlansResult.error
      ? []
      : ((kidsPlansResult.data ?? []) as InstagramKidsPlanRow[]).map((plan) => ({
